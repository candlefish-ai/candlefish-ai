...
class EnhancedReviewer:
    ...
    def filter_files_by_depth(self, files: List, depth: str) -> List:
        ...
        for file in files:
            ...
            if config['skip_tests'] and any(pattern in file.filename.lower() 
                                          for pattern in ['test', 'spec', '__tests__']):
                continue
            
            # Skip docs if configured
            if config['skip_docs'] and any(pattern in file.filename.lower() 
    ...
    def parse_review_command(self, comment_body: str) -> Dict:
        ...
        if command_match:
            ...
            for option in options:
                if option in ['security', 'performance', 'architecture', 'comprehensive']:
    ...
    def format_enhanced_review(self,
                             review_content: str,
                             pr_number: int,
                             metadata: Dict) -> str:
