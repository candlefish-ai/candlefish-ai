...
class RTPMAPIUser(FastHttpUser):
    ...
    def login(self):
        ...
        login_data = {"username": self.user_id, "password": "load_test_password"}
    ...
    def query_metrics(self):
        ...
        with self.client.get(
            "/api/v1/metrics/query", params=params, headers=self.get_headers()
        ) as response:
            if response.status_code != 200:
    ...
    def get_aggregated_metrics(self):
        ...
        with self.client.get(
            "/api/v1/metrics/aggregated", params=params, headers=self.get_headers()
        ) as response:
            if response.status_code != 200:
    ...
    def get_active_alerts(self):
        ...
        with self.client.get("/api/v1/alerts/active", headers=self.get_headers()) as response:
            if response.status_code != 200:
