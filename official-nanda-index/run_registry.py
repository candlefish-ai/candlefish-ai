...
def check_port_80():
    ...
    try:
        ...
        result = subprocess.run(["sudo", "lsof", "-i", ":80"], capture_output=True, text=True)
        ...
        if result.returncode == 0:
            ...
            for service in ["apache2", "nginx", "httpd"]:
                try:
                    # First try systemctl
                    subprocess.run(
                        ["sudo", "systemctl", "stop", service], stderr=subprocess.DEVNULL
                    )
    ...
    try:
        ...
        if shutil.which("certbot") is None:
            ...
            result = subprocess.run(
                ["sudo", "apt-get", "install", "-y", "certbot"], capture_output=True, text=True
            )
        ...
        try:
            ...
            check_process = subprocess.run(
                ["pgrep", "-f", "registry.py"], capture_output=True, text=True
            )
    ...
    try:
        ...
        if "tunnels" in data and len(data["tunnels"]) > 0:
            for tunnel in data["tunnels"]:
                ...
                if "addr" in tunnel.get("config", {}) and tunnel["config"]["addr"].endswith(
                    ":6900"
                ):
...
def main():
    ...
    parser.add_argument(
        "--port", type=int, default=6900, help="Registry server port (default: 6900)"
    )
    ...
    cert_dir = os.path.join(os.path.expanduser("~"), "certificates")
    cert_files_exist = os.path.exists(os.path.join(cert_dir, "fullchain.pem")) and os.path.exists(
    ...
    print("Saved URL to registry_url.txt")
