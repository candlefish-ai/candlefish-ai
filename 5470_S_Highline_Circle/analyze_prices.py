...
if 'Price (Designer Invoice)' in df.columns:
    print(f'Items with Designer Invoice Price: {df["Price (Designer Invoice)"].notna().sum()} out of {len(df)}')
...
designer_price_col = df['Price (Designer Invoice)'].fillna(0) if 'Price (Designer Invoice)' in df.columns else 0
