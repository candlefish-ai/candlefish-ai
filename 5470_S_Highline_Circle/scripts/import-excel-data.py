...
# Database connection
DATABASE_URL = os.getenv('DATABASE_URL', 'postgresql://highline:rtpm_secure_password_123@localhost:5434/highline_inventory')
    ...
    for _, row in inventory_df.iterrows():
        if pd.notna(row.get('Room')) and pd.notna(row.get('Item')):
            ...
            if room_id:
                ...
                price = row.get('Price (Designer Invoice)') if pd.notna(row.get('Price (Designer Invoice)')) else None
    ...
    for _, row in bloom_df.iterrows():
        if pd.notna(row.get('Room')) and pd.notna(row.get('Plant/Planter')):
            ...
            if room_id:
                ...
                category = 'Plant (Indoor)' if 'Indoor' in str(row.get('Indoor/Outdoor', '')) else 'Outdoor Planter/Plant'
                ...
                cursor.execute(
                    ...
                    (
                        ...
                        row.get('Line Total', 0) / row.get('Qty', 1) if row.get('Qty', 1) > 0 else row.get('Line Total', 0),
                        row.get('Qty', 1),
                        'Bloom & Flourish',
