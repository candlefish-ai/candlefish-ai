...
def setup_database():
    ...
    print(
        f"Setting up database: {database_url.split('@')[1] if '@' in database_url else 'unknown'}"
    )
    ...
    try:
        ...
        if not os.path.exists(excel_path):
            source_excel = os.path.join(
                os.path.dirname(__file__), "..", "5470_furnishings_inventory.xlsx"
            )
