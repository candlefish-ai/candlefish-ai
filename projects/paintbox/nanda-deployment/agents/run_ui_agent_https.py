...
def list_agents():
    ...
    try:
        ...
        return jsonify(
            {"error": f"Failed to get agent list: {response.text}"}
...
def receive_message():
    ...
    try:
        ...
        message_file = "latest_message.json"
...
def main():
    ...
    os.environ["UI_CLIENT_URL"] = (
        f"{api_url}/api/receive_message"  # f"https://chat2.nanda-registry.com:{api_port}/api/receive_message"
    )
    ...
    bridge_process = subprocess.Popen(
        ["python3", "agent_bridge.py"], stdout=log_file, stderr=log_file
    )
