...
class PaintboxMonitorAgent(AgentBridge):
    ...
    def trigger_optimization(self):
        ...
        if time.time() - self.last_optimization > 300:  # 5 min cooldown
            print("🚨 Memory high, triggering optimization...")
            requests.post(f"{self.staging_url}/api/memory/optimize", json={"level": "standard"})
            self.last_optimization = time.time()

    def trigger_healing(self):
        """Attempt to heal unhealthy service"""
        print("🏥 Service unhealthy, attempting healing...")
        subprocess.run(["fly", "machine", "restart", "d89642eae79628", "--app", "paintbox-staging"])

    def log_metrics(self, health):
        """Log metrics to file"""
        with open(f"{os.getenv('LOG_DIR')}/monitor_metrics.json", "a") as f:
            json.dump({"timestamp": datetime.now().isoformat(), "health": health}, f)
            f.write("\n")
