...
class NaturalLanguageParser:
    ...
    COMMAND_PATTERNS = {
        "safe_deploy": [r"deploy.*safe", r"careful.*deploy", r"extra.*security", r"be.*careful"],
        "fast_deploy": [r"deploy.*fast", r"quick.*deploy", r"hurry", r"asap", r"emergency"],
        "repeat_deploy": [r"same as", r"like last", r"repeat.*deploy", r"do.*again"],
        "rollback": [r"rollback", r"undo", r"revert", r"fix.*broke"],
        "test_only": [r"just.*test", r"only.*test", r"dry.*run", r"simulate"],
    }
...
class EnhancedDeploymentOrchestrator:
    ...
    def get_deployment_insights(self) -> str:
        ...
        avg_duration = (
            sum(m.metrics.get("duration_seconds", 0) for m in self.memory_system) / total
            if total > 0
            else 0
        )
