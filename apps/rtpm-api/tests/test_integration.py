...
class TestDatabaseIntegration:
    ...
    def test_agent_crud_operations(self, test_db_engine, sample_agent):
        ...
        with test_db_engine.connect() as conn:
            ...
            # Read agent
            result = conn.execute(text("SELECT * FROM agents WHERE id = :id"), {'id': sample_agent['id']})
            row = result.fetchone()
            ...
            # Verify update
            result = conn.execute(text("SELECT status FROM agents WHERE id = :id"), {'id': sample_agent['id']})
            row = result.fetchone()
            ...
            # Verify deletion
            result = conn.execute(text("SELECT COUNT(*) FROM agents WHERE id = :id"), {'id': sample_agent['id']})
    ...
    def test_concurrent_metric_ingestion(self, client):
        ...
        for i in range(20):
            thread = threading.Thread(
                target=ingest_metric,
                args=(f"agent-{i % 5}", 50.0 + i)
            )
