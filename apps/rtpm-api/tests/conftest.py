...
def sample_agents_list():
    ...
    for i in range(50):
        agent = {
            ...
            "capabilities": random.sample(["monitoring", "analysis", "reporting", "alerting"], k=random.randint(1,3)),
...
def sample_large_agents_list():
    ...
    for i in range(1000):
        agent = {
            ...
            "capabilities": random.sample(["monitoring", "analysis", "reporting", "alerting", "optimization"], k=random.randint(1,4)),
            "lastSeen": (datetime.utcnow() - timedelta(minutes=random.randint(0, 1440))).isoformat(),
            "region": random.choice(regions),
            "platform": random.choice(platforms),
            "tags": [f"tag-{random.randint(1,10)}", f"env-{random.choice(['prod', 'staging', 'dev'])}"]
        }
        agents.append(agent)
...
def sample_alert_history():
    ...
    for i in range(20):
        alert = {
            ...
            "resolvedAt": None if random.random() < 0.3 else (base_time + timedelta(hours=random.randint(0, 168), minutes=random.randint(10, 120))).isoformat(),
        ...
        if alert["acknowledged"] and alert["acknowledgedBy"]:
            alert["acknowledgedAt"] = (base_time + timedelta(hours=random.randint(0, 168), minutes=random.randint(5, 60))).isoformat()
...
def validate_metrics_data(metrics_data):
    ...
    required_fields = ["agentId", "timestamp", "cpu", "memory", "requestRate", "errorRate", "responseTime"]
    for field in required_fields:
